...
**On NoAuth mode protection:**
Implement a check at application startup:
...
if (
  ...
  if (process.env.ALLOW_NOAUTH_IN_PRODUCTION !== "true") {
    throw new Error(
      "NoAuth mode is not allowed in production. Set ALLOW_NOAUTH_IN_PRODUCTION=true to override.",
    );
  }
  console.warn(
...
**On documentation:**
Create a separate document `docs/security/noauth-mode.md` explaining:

- What NoAuth mode is
- When to use it (development and testing only)
- Risks of use
- How to enable/disable

**On next steps:**
After approving the specification, it is recommended to:

1. Choose a working name for the project
2. Create the monorepo structure
3. Implement the thinnest slice: repository creation + HTTP push/pull
4. Iteratively build up functionality

---

**The specification is ready for approval.** Are there any questions or revisions?
