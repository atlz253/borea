...
test("create pull request via web UI and merge it", async ({ page }) => {
	...
	try {
		...
		await page.getByLabel("Target branch").click();
		const targetOption = page.getByRole("option", { name: defaultBranch });
		await targetOption.click();

		await page.getByRole("button", { name: /Create pull request/i }).click();

		await page.waitForURL(/\/repositories\/$repoName\/pulls\/\d+/);
		await page.waitForLoadState("load");
		...
		await expect(page.getByText(/merged/i)).toBeVisible({ timeout: 15_000 });
